...
- The primer ends with a short "Other platforms" section: the bridge is gated on `A11Y_HAVE_PRISM`, Prism ships macOS and Linux zips under the same tag, a port is a cmake branch plus a play-test, and the base port's macOS packaging already bundles and ad-hoc signs every dylib. Point at `.scratch/macos-port/spec.md` for the worked plan.
