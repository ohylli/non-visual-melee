...
2026-09-23, code review: the "three lines" grep criterion could never be met as written, since the `#include` line contains no `pc_a11y_`. The footprint is the intended three touches in `main.c` (include, init call, shutdown call). CLAUDE.md now gives the inventory command as `grep -rn a11y src CMakeLists.txt --exclude-dir=a11y`, which shows all of them plus the cmake include. The review also added a distinct log line for an `announce` before `init` (previously reported as an off-thread call) and folded the seven log-switch checks into one `Speech::log` helper.
